...
type JUnitsForEvents interface {
	...
	JUnitsForEvents(events monitorapi.Intervals, duration time.Duration, kubeClientConfig *rest.Config, testSuite string, m *monitor.Monitor) []*junitapi.JUnitTestCase
}
...
type JUnitForEventsFunc func(events monitorapi.Intervals, duration time.Duration, kubeClientConfig *rest.Config, testSuite string, m *monitor.Monitor) []*junitapi.JUnitTestCase

// JUnitsForEvents for JUnitForEventsFunc calls the function with parameters passed and returns
// the resulting JUnitTestCases.
func (fn JUnitForEventsFunc) JUnitsForEvents(events monitorapi.Intervals, duration time.Duration, kubeClientConfig *rest.Config, testSuite string, m *monitor.Monitor) []*junitapi.JUnitTestCase {
	return fn(events, duration, kubeClientConfig, testSuite, m)
}
...
// JUnitForAllEvents implements the JUnitsForEvents interface.
func (a JUnitsForAllEvents) JUnitsForEvents(events monitorapi.Intervals, duration time.Duration, kubeClientConfig *rest.Config, testSuite string, m *monitor.Monitor) []*junitapi.JUnitTestCase {
	var all []*junitapi.JUnitTestCase
	for _, obj := range a {
		if obj == nil {
			continue
		}
		results := obj.JUnitsForEvents(events, duration, kubeClientConfig, testSuite, m)
